...
int main(int argc, char *argv[]) {
    ...
    options.add_options()
        ...
        ("weight_dist", "activation to use (relu/tanh)", cxxopts::value<std::string>()->default_value("gaussian"))
  ...
  string weight_dist = result["weight_dist"].as<string>();
  ...
  ofstream datafile;
    ...
    filename += string("_fun_samples.txt");
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int l=0; l< n_layers; l++) {
        ...
        else if (weight_dist == "uniform")
            gen_w = std::sqrt(12)*sigmaw/std::sqrt(nhs[l])*(arma::randu<vec>( nhs[l]*nhs[l+1] ) - 0.5);
        std::copy(gen_w.begin(),gen_w.end(),it);
        it+=nhs[l]*nhs[l+1];
        if (weight_dist == "gaussian")
            gen_b = sigmab*arma::randn<vec>( nhs[l+1] );
        else if (weight_dist == "uniform")
            gen_b = std::sqrt(12)*sigmab*(arma::randu<vec>( nhs[l+1] ) - 0.5);
        std::copy(gen_b.begin(),gen_b.end(),it);
        it+=nhs[l+1];
    }
    found_fun = pheno(base, gen, nhs, n_layers, centered, activation);
    datafile << found_fun << "\n";
  }
