...
int main(int argc, char const *argv[]) {
  ...
  ofstream datafile;
  datafile.open (string(argv[1])+string("_")+to_string(world_rank)+string("_")+comp+string("_")+string("abi_sample_net.txt"));
  ...
  for (int i = 0; i < stoi(string(argv[2])); i++) {
    ...
    gen_w = sigmaw/std::sqrt(nh_1)*(arma::randn<vec>( nh_1*nh_2 ));
    gen.insert(gen.end(),gen_w.begin(), gen_w.end());
    gen_b = sigmab*arma::randn<vec>( nh_2 );
    gen.insert(gen.end(),gen_b.begin(), gen_b.end()); 

    gen_w = sigmaw/std::sqrt(nh_2)*(arma::randn<vec>( nh_2*n_out ));
    gen.insert(gen.end(),gen_w.begin(), gen_w.end());
