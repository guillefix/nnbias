...
int main (int argc, char** argv) {
  map<string, tuple<double,double,double,int>> inputs_stats;
  ofstream myfile;
 inputs_stats.clear();
...
cout << "finished reading" << "\n";
     ...
     myfile.open (argv[2]);
      ...
      {
        tuple<double,double,double,int> tuple = iterator->second;
        string output = iterator->first;
        //output complexity, min input complexity, max input complexity, mean input complexity, frequency
          myfile << to_string(calc_KC(output.c_str())) + " " + to_string(std::get<1>(tuple)) + " " + to_string(std::get<2>(tuple)) + " " + to_string(std::get<0>(tuple)/std::get<3>(tuple)) + " " + to_string(std::get<3>(tuple)) + "\n";
      }
     myfile.close();
}
