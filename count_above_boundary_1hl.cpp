...
int main(int argc, char const *argv[]) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    gen_w = sigmaw/std::sqrt(nh_1)*(arma::randn<vec>( nh_1*n_out ));
    gen.insert(gen.end(),gen_w.begin(), gen_w.end());
    gen_b = sigmab*arma::randn<vec>( n_out );
    gen.insert(gen.end(),gen_b.begin(), gen_b.end()); 


    t = num_above_boundary(n_input, gen);
    fracs[t] += 1;
    ...
    ofstream myfile;
     myfile.open(string(argv[1])+to_string(base)+string("_")+to_string(n_input)+string("_")+to_string(world_rank)+string("_")+string("1hl_t_counts.txt"));
     int count;
